<template>
  <h3>Propiedades Computadas</h3>
  <input type="text" v-model="valor">
  <!-- <ResultadoSection v-show="mostrarElemento" :resultado="valorComputado"></ResultadoSection> -->
  <ResultadoSection v-if="mostrarElemento" :resultado="valorComputado"></ResultadoSection>
  <div v-else-if="isNaN(valorComputado)">
    El valor no es un número
  </div>
  <div v-else>
    Aqui no se muestra nada
  </div>
</template>

<script setup>
  import { ref, computed } from 'vue';
  import ResultadoSection from '../components/ResultadoSection.vue';
  const valor = ref('');
  const valorComputado = computed(() => {
    return valor.value * 3;
  });
  const mostrarElemento = computed(() => {
    return valor.value > 10;
  });
</script>

<style scoped>
</style>
